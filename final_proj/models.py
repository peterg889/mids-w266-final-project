...
def softmax_output_layer(h_, labels_, num_classes):
    ...
    with tf.variable_scope("Logits"):
        ...
        d =  h_.shape[1]
        W_out_ = tf.get_variable(name='W_out', shape=(d, num_classes), initializer=tf.random_normal_initializer())
    ...
    with tf.name_scope("Softmax"):
        ...
        loss_ = tf.reduce_mean(softmax_, axis=0)
        #### END(YOUR CODE) ####
...
def BOW_encoder(ids_, ns_, V, embed_dim, hidden_dims, dropout_rate=0,
    ...
    summed_embeddings_ = tf.reduce_sum(masked_, axis=1)

    # Build a stack of fully-connected layers
    h_ = fully_connected_layers(summed_embeddings_, hidden_dims)
